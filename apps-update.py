...
class XiaoHeiHe:
    def __init__(self):
        self.app_info = MyJson(f'{yesterday_str}-apps.json')
        self.pbar = tqdm(delay=1)
        self.pbar.delay = 0
        self.xiao_hei_he = MyJson(f'{yesterday_str}-xiaoheihe.json')
    ...
    def task(self, app_id, app_info):
        try:
            ...
            if 'common' in app_info:
                ...
                update_time = app_info['update_time']
            cname = None
            about = None
            score = None
            release_date = None
            ...
            info = {'type': type_, 'name': name, 'cname': cname, 'tags': tags,
                    'score': score, 'release_date': release_date, 'update_time': update_time}
            wait = 60
            with lock:
...
def get_app_info(repo):
    app = MyJson(f'{yesterday_str}-apps.json')
    app_update = {}
    steam = SteamClient()
    ...
    while (pagesRemaining):
        ...
        res = r.json()
        for i in res:
            if 'data' not in i['ref'] and 'main' not in i['ref']:
                update_time = datetime.strptime(i['timestamp'], '%Y-%m-%dT%H:%M:%SZ')
                update_time_local = update_time + timedelta(hours=8)
                if yesterday_start_time <= update_time_local.timestamp() <= yesterday_end_time:
                    app_id = i['ref'].replace('refs/heads/', '')
                    app_update[f'{app_id}'] = update_time_local.strftime('%Y-%m-%d %H:%M:%S')
                    app_id_list.append(int(app_id))
                if update_time_local.timestamp() < yesterday_start_time:
                    break
        link_header = None
        if 'link' in r.headers:
            link_header = r.headers['link']

        pagesRemaining = link_header and 'rel=\"next\"' in link_header
        if pagesRemaining:
            str = re.finditer(nextPattern, link_header)
            for e in str:
                url = e.group().replace('>', '').replace('<', '')
    ...
    while app_id_list[count:count + 300]:
        ...
        if fresh_resp:
            for app_id, info in fresh_resp['apps'].items():
                ...
                app_info_dict[int(app_id)]['update_time'] = app_update[f'{app_id}']
...
def export_xlsx(save_path='.'):
    ...
    ws.append(['app id', '游戏名', '中文名', '更新时间', '标签', '类型', '评分', '发行日期', '简介'])
    for i, info in sorted(MyJson(f'{yesterday_str}-xiaoheihe.json').items(), key=lambda x: int(x[0])):
        try:
            ws.append([i, info['name'], info['cname'], info['update_time'], ','.join(info['tags']), info['type'],
                       info['score'],
                       info['release_date'], info['about']])
        except IllegalCharacterError:
            ws.append([i, info['name'], info['cname'], info['update_time'], ','.join(info['tags']), info['type'], info['score'],
                       info['release_date'], ILLEGAL_CHARACTERS_RE.sub('', info['about'])])
    if save_path.is_dir():
        save_path = save_path / f'{yesterday_str}-apps.xlsx'
    workbook.save(save_path)


parser = argparse.ArgumentParser()
parser.add_argument('-r', '--repo', default='lls7890/Repository')
